...
AS
	...
	IS
		...
		SELECT DISTINCT * FROM (
			SELECT * FROM (
				SELECT
					C.RAMOAS RAMO,C.POLIZAAS POLIZA,C.SERIE SERIE,C.RECLAMAS RECLAM,C.NUMCERTAS NUMCERT,C.NUMRECAS NUMREC,
					C.CODCOBAS CODCOB,'CURSOR_1' "CURSOR",PM.TB_REF_TIPO_SUSCRIPCION (C.RAMOAS) TIPOSUSC
			...
			WHERE EXISTS (
				SELECT DISTINCT RAMO,POLIZA FROM HJPF01 B
				WHERE A.RAMO = B.RAMO AND A.POLIZA = B.POLIZA
			)
			UNION ALL
			SELECT
				C.RAMO,C.POLIZA,A.SERIE,A.RECLAMAS,A.NUMCERTAS NUMCERT,C.NUMREC,A.CODCOBAS CODCOB,'CURSOR_2' "CURSOR",
				PM.TB_REF_TIPO_SUSCRIPCION (C.RAMO) TIPOSUSC
			...
			UNION ALL
			SELECT
				A.RAMOAS RAMO,A.POLIZAAS POLIZA,A.SERIE SERIE,A.RECLAMAS RECLAM,A.NUMCERTAS NUMCERT,A.NUMRECAS NUMREC,
				A.CODCOBAS CODCOB,'CURSOR_3' "CURSOR",PM.TB_REF_TIPO_SUSCRIPCION (A.RAMOAS) TIPOSUSC
	...
	BEGIN
		...
		FOR I IN (
			...
			WHERE SUBSTR(PM.BUSCA_PRODUCTO_PLAN (RAMO,POLIZA,NUMCERT),1,4) IN ('AUTI')
		)
		LOOP
			FOR J IN (
				SELECT * FROM (
					SELECT 32 RAMO FROM DUAL
					UNION
					SELECT 33 RAMO FROM DUAL
					UNION
					SELECT 34 RAMO FROM DUAL
					UNION
					SELECT 35 RAMO FROM DUAL)
			)
			LOOP
				INSERT INTO SINIESTROS_RECIBO_MULTIRAMO
				VALUES (J.RAMO,I.POLIZA,I.NUMREC);
			END LOOP;
			COMMIT;
		END LOOP;
	...
	IS
		...
		IS
		SELECT 
			A.RAMO,A.POLIZA,A.NUMREC,B.CODCOB,BIECOB,DECODE(NVL(PRIMAR,1),0,1,NVL (PRIMAR,1)) PRIMAMONEDA,
			DECODE(NVL(SUMASE,1),0,1,NVL (SUMASE,1)) SUMAASEGMONEDA
		FROM SINIESTROS_RECIBO_MULTIRAMO A,HJPF05_REN B
		WHERE A.RAMO = B.RAMO
		AND A.POLIZA = B.POLIZA
		AND A.NUMREC = B.NUMREC
		ORDER BY RAMO,POLIZA,NUMREC,CODCOB;
	BEGIN
		...
		LOOP
			INSERT INTO EQ_SINIESTRO_REN (
				RAMO,POLIZA,NUMREC,NUMCERT,CODCOB,BIECOB,PRIMAMONEDA,SUMAASEGMONEDA,MIGRA,RAZONMIGRA
			)
			VALUES (
				I.RAMO,I.POLIZA,I.NUMREC,1,I.CODCOB,I.BIECOB,I.PRIMAMONEDA,I.SUMAASEGMONEDA,'S','N/A'
			);
			COMMIT;
		END LOOP;
		--
		FOR I IN (SELECT DISTINCT RAMO,POLIZA,NUMREC FROM EQ_SINIESTRO_REN)
		LOOP
			...
			FECING = PM.CONVIERTE_DIAMESANO_FECHA (
				(SELECT DIAEMI FROM HJPF02_REN WHERE RAMO = I.RAMO AND POLIZA = I.POLIZA AND NUMREC = I.NUMREC),
				(SELECT MESEMI FROM HJPF02_REN WHERE RAMO = I.RAMO AND POLIZA = I.POLIZA AND NUMREC = I.NUMREC),
				(SELECT ANOEMI FROM HJPF02_REN WHERE RAMO = I.RAMO AND POLIZA = I.POLIZA AND NUMREC = I.NUMREC)
			),
			...
			NUMREN = PM.CALCULA_NUMERO_RENOVACIONES (PM.CONVIERTE_DIAMESANO_FECHA ( 
				...
				(SELECT MESDES FROM HJPF02_REN WHERE RAMO = I.RAMO AND POLIZA = I.POLIZA AND NUMREC = I.NUMREC),
				(SELECT ANODES FROM HJPF02_REN WHERE RAMO = I.RAMO AND POLIZA = I.POLIZA AND NUMREC = I.NUMREC)),
				(SELECT PM.CONVIERTE_DIAMESANO_FECHA (DIACOM,MESCOM,ANOCOM) FROM HJPF01_REN WHERE RAMO = I.RAMO AND POLIZA = I.POLIZA)
			)
			WHERE RAMO = I.RAMO
			AND POLIZA = I.POLIZA
			AND NUMREC = I.NUMREC;
		...
		END LOOP;
		--
		FOR I IN (SELECT DISTINCT RAMO,POLIZA,CODCOB FROM EQ_SINIESTRO_REN WHERE MIGRA = 'S')
		LOOP
			BEGIN
	...
	BEGIN
		UPDATE EQ_SINIESTRO_REN
		SET MIGRA = 'N',
			RAZONMIGRA = 'NO tiene Fechas o Fechas Erronea'
		WHERE MIGRA = 'S'
		AND (
			FECEMI IS NULL OR
	...
	BEGIN
		FOR I IN (SELECT DISTINCT RAMO,POLIZA FROM EQ_SINIESTRO_REN WHERE MIGRA = 'S')
		LOOP
			v_CODPROD := NULL;
			V_CODPLAN := NULL;
			V_REVPLAN := NULL;
		...
		END LOOP;
		--
		FOR I IN (SELECT DISTINCT RAMO,POLIZA,NUMREC FROM EQ_SINIESTRO_REN WHERE MIGRA = 'S')
		LOOP
			UPDATE EQ_SINIESTRO_REN
			SET STSSIT = (
				SELECT NVL (TRIM (STSSIT),'0') FROM HJPF02_REN WHERE RAMO = I.RAMO AND POLIZA = I.POLIZA AND NUMREC = I.NUMREC
			),
			SUMAASEG = (
				SELECT DECODE (TOTSAS,0,1,TOTSAS) FROM HJPF02_REN WHERE RAMO = I.RAMO AND POLIZA = I.POLIZA AND NUMREC = I.NUMREC
			),
			MONTOPRIMA = (
				SELECT DECODE (TOTPRI,0,1,TOTPRI) FROM HJPF02_REN WHERE RAMO = I.RAMO AND POLIZA = I.POLIZA AND NUMREC = I.NUMREC
			),
			TASACAMBIO = (
				SELECT DECODE (TIPCAM,0,1,TIPCAM) FROM HJPF02_REN WHERE RAMO = I.RAMO AND POLIZA = I.POLIZA AND NUMREC = I.NUMREC
			)
			WHERE MIGRA = 'S'
			AND RAMO = I.RAMO
			AND POLIZA = I.POLIZA
		...
		END LOOP;
		--
		FOR I IN (SELECT DISTINCT RAMO,POLIZA,NUMREC,STSSIT,PRIMAMONEDA FROM EQ_SINIESTRO_REN WHERE MIGRA = 'S')
		LOOP
			BEGIN
				SELECT (
					CASE
		...
		END LOOP;
		--
		FOR I IN (SELECT DISTINCT CODPROD,CODPLAN,REVPLAN,CODRAMOCERT,CODCOBERT FROM EQ_SINIESTRO_REN WHERE MIGRA = 'S')
		LOOP
			UPDATE EQ_SINIESTRO_REN
			SET CODRAMOREA = (
	...
	IS
		...
		vDUMMY 			NUMBER;
	BEGIN
		FOR I IN (SELECT DISTINCT RAMO,POLIZA,CODPROD FROM EQ_SINIESTRO_REN WHERE MIGRA = 'S')
		LOOP
			vDUMMY := 0;
			V_TIPOID := NULL;
			V_NUMID := NULL;
	...
	IS
		...
		SELECT
			...
			NULL INDSUMADED,NULL PORCDED,NULL MTODEDMIN,NULL BASEDED,NULL PORCDESC,NULL ORDEN,NULL PORCREEMB,NULL PORCMINASEG
		FROM EQ_SINIESTRO_REN
		WHERE MIGRA = 'S';
		--
		...
		SELECT
			...
			PRIMAMONEDA PRIMA,FECING FECING,NULL FECEXC,CODPLAN CODPLAN,REVPLAN REVPLAN,CODPARENT CODPARENT,NULL FECANTASEG
		FROM EQ_SINIESTRO_REN
		WHERE MIGRA = 'S'
		AND RAMO IN (SELECT TO_NUMBER (ACARGU) FROM CTRALTAB WHERE ACTIAR = 4855 AND TRIM (ACARGU) <> '**');
		--
		...
		IS
		SELECT
			RAMO RAMO_AS,POLIZA POLIZA_AS,NUMCERT,CODRAMOCERT,TIPOID,NUMID,DVID,CODPLAN,REVPLAN,CODCOBERT,
			STSCOBERT,NUMREC,SUMAASEGMONEDA SUMAASEG,TASACAMBIO TASA,PRIMAMONEDA PRIMA,CODMONEDA,SUMAASEGMONEDA,
			PRIMAMONEDA,FECINIVALID,FECFINVALID,NULL INDINCREN,NULL ORDEN,NULL PORCREEMB
		FROM EQ_SINIESTRO_REN
		WHERE MIGRA = 'S'
		AND RAMO IN (SELECT TO_NUMBER (ACARGU) FROM CTRALTAB WHERE ACTIAR = 4855 AND TRIM (ACARGU) <> '**');
		--
		...
		IS
		SELECT
			RAMO RAMO_AS,POLIZA POLIZA_AS,NUMCERT NUMCERT,CODRAMOCERT CODRAMOCERT,CODMONEDA CODMONEDA,CLASEBIEN CLASEBIEN,
			CODBIEN CODBIEN,STSBIEN STSBIEN,NULL MTOVALBIEN,NULL MTOVALBIENMONEDA,NULL PORCRIESGO,NULL TIPO1ERIESGO,
			NULL INDMEGARIESGO,NULL DESCBIEN,NULL PORCPERIND,NULL INDAUMVAL,SUMAASEG SUMAASEG,NULL IDEBIENPLNS,NULL IDEBIENCASLO
		FROM EQ_SINIESTRO_REN
		WHERE MIGRA = 'S'
		AND RAMO IN (SELECT TO_NUMBER (ACARGU) FROM CTRALTAB WHERE ACTIAR = 4852 AND TRIM (ACARGU) <> '**');
		--
		CURSOR CUR_COBERT_BIEN
		IS
		SELECT
			CODPLAN,REVPLAN,CODCOBERT,RAMO RAMO_AS,POLIZA POLIZA_AS,NUMCERT,CODBIEN,CODRAMOCERT,STSCOBERT,SUMAASEG,
			TASACAMBIO TASA,MONTOPRIMA PRIMA,CODMONEDA,SUMAASEGMONEDA,PRIMAMONEDA,FECINIVALID,FECFINVALID,NULL PORCDED,
			NULL MTODEDMIN,NULL BASEDED,NULL PORCDESC,NULL INDSUMADED,NULL ORDEN,NULL PORCREEMB,NULL PORCMINASEG,TASACAMBIO TASABRUTA
		FROM EQ_SINIESTRO_REN
		WHERE MIGRA = 'S'
		AND RAMO IN (SELECT TO_NUMBER (ACARGU) FROM CTRALTAB WHERE ACTIAR = 4852 AND TRIM (ACARGU) <> '**');
		--
	...
	BEGIN
		...
		LOOP
			...
			IF CUR_VIG_POLIZA%NOTFOUND THEN
				...
				VALUES (
					NULL,I.CODPOL,I.NUMPOL,I.NUMREN,I.RAMO_AS,I.POLIZA_AS,I.CODPROD,I.STSPOL,I.TIPOID,I.NUMID,
					I.DVID,I.FECREN,I.FECINIVIG,I.FECFINVIG,I.TIPOVIG,I.TIPOPDCION,I.TIPOFACT,I.INDCOA,I.CODFORMFCION,
					I.CODOFIEMI,I.CODOFISUSC,I.CODMONEDA,I.TIPOSUSC,I.CODFORMPAGO,I.FECULTFACT,I.INDRENOVLOTE,
					I.CODCONDCOBRO,I.RESPCOB,I.CODGRP,I.FECINGPOL,I.FECEMI,I.CODINTER,NULL,NULL,NULL
				);
				COMMIT;
			END IF;
		...
		LOOP
			...
			IF CUR_VIG_RECIBO%NOTFOUND THEN
				...
				VALUES (
					...
					I.STSREC,I.STSRECFIN,I.MONTOPRIMAMON,I.MTOCOMISIONMON,I.CODOFICOB,I.FECCOBRO,NULL
				);
				COMMIT;
			END IF;
		...
		LOOP
			...
			IF CUR_VIG_CERTIFICADO%NOTFOUND THEN
				...
				VALUES (
					...
					I.FECANTCERT,I.CODRAMOCERT,I.FECINIVALID,I.FECFINVALID,I.CODCUMULO,I.CODPLAN,I.REVPLAN,NULL,NULL
				);
				COMMIT;
			END IF;
		...
		LOOP
			...
			IF CUR_VIG_COBERT_CERT%NOTFOUND THEN
				...
				VALUES (
					I.RAMO_AS,I.POLIZA_AS,I.NUMCERT,I.CODRAMOCERT,I.CODPLAN,I.REVPLAN,I.CODCOBERT,I.STSCOBERT,
					I.SUMAASEGMONEDA,I.TASA,I.PRIMAMONEDA,I.CODMONEDA,I.SUMAASEG,I.PRIMA,I.FECINIVALID,I.FECFINVALID,
					I.INDSUMADED,I.PORCDED,I.MTODEDMIN,I.BASEDED,I.PORCDESC,I.ORDEN,I.PORCREEMB,I.PORCMINASEG
				);
				COMMIT;
			END IF;
		...
		LOOP
			...
			IF CUR_VIG_COBERT_CERT_REC%NOTFOUND THEN
				...
				VALUES (
					...
					I.PRIMA,I.FECINIVALID,I.FECFINVALID,I.PRIMAANUAL,I.CODRAMOCERT,I.PRIMAMON,NULL
				);
				COMMIT;
			END IF;
		...
		LOOP
			...
			IF CUR_VIG_ASEGURADO%NOTFOUND THEN
				...
				VALUES (
					...
					I.INDASEGTITULAR,I.MTOTOTRECAMONEDA,I.MTOTOTDCTOMONEDA,I.CODMONEDA,I.PRIMA,
					I.FECING,I.FECEXC,I.CODPLAN,I.REVPLAN,I.CODPARENT,I.FECANTASEG,NULL,NULL
				);
				COMMIT;
			END IF;
		...
		LOOP
			...
			IF CUR_VIG_COBERT_ASEG%NOTFOUND THEN
				...
				VALUES (
					...
					I.CODCOBERT,I.STSCOBERT,I.NUMREC,I.SUMAASEG,I.TASA,I.PRIMA,I.CODMONEDA,I.SUMAASEGMONEDA,
					I.PRIMAMONEDA,I.FECINIVALID,I.FECFINVALID,I.INDINCREN,I.ORDEN,I.PORCREEMB,NULL,NULL,NULL
				);
				COMMIT;
			END IF;
		...
		LOOP
			...
			IF CUR_VIG_COBERT_ASEG_REC%NOTFOUND THEN
				...
				VALUES (
					I.RAMO_AS,I.POLIZA_AS,I.NUMCERT,I.CODRAMOCERT,I.TIPOID,I.NUMID,I.DVID,I.CODPLAN,I.REVPLAN,
					I.CODCOBERT,I.NUMREC,I.SUMAASEG,I.TASA,I.PRIMA,I.FECINIVALID,I.FECFINVALID,I.PRIMAANUAL,NULL
				);
				COMMIT;
			END IF;
		...
		LOOP
			...
			IF CUR_VIG_BIEN_CERT%NOTFOUND THEN
				...
				VALUES (
					I.RAMO_AS,I.POLIZA_AS,I.NUMCERT,I.CODRAMOCERT,I.CODMONEDA,I.CLASEBIEN,I.CODBIEN,I.STSBIEN,
					I.MTOVALBIEN,I.MTOVALBIENMONEDA,I.PORCRIESGO,I.TIPO1ERIESGO,I.INDMEGARIESGO,I.DESCBIEN,
					I.PORCPERIND,I.INDAUMVAL,I.SUMAASEG,I.IDEBIENPLNS,I.IDEBIENCASLO
				);
				COMMIT;
			END IF;
		...
		LOOP
			...
			IF CUR_VIG_COBERT_BIEN%NOTFOUND THEN
				...
				VALUES (
					I.CODPLAN,I.REVPLAN,I.CODCOBERT,I.RAMO_AS,I.POLIZA_AS,I.NUMCERT,I.CODBIEN,I.CODRAMOCERT,I.STSCOBERT,
					I.SUMAASEG,I.TASA,I.PRIMA,I.CODMONEDA,I.SUMAASEGMONEDA,I.PRIMAMONEDA,I.FECINIVALID,I.FECFINVALID,
					I.PORCDED,I.MTODEDMIN,I.BASEDED,I.PORCDESC,I.INDSUMADED,I.ORDEN,I.PORCREEMB,I.PORCMINASEG,I.TASABRUTA
				);
				COMMIT;
			END IF;
		...
		LOOP
			...
			IF CUR_VIG_COBERT_BIEN_REC%NOTFOUND THEN
				...
				VALUES (
					I.RAMO_AS,I.POLIZA_AS,I.NUMCERT,I.CODRAMOCERT,I.CODBIEN,I.CODPLAN,I.REVPLAN,I.CODCOBERT,I.NUMREC,
					I.SUMAASEG,I.TASA,I.PRIMA,I.FECINIVALID,I.FECFINVALID,I.PRIMAANUAL,I.PRIMAMON,NULL
				);
				COMMIT;
			END IF;
	...
	IS
		...
		SELECT DISTINCT * FROM (
			...
			FROM REPF14_REN A,(
				SELECT RAMO,POLIZA,NUMREC,SUM (PRIMA) PRIMA FROM REPF14_REN
	...
	BEGIN
		...
		LOOP
			...
			BEGIN
				...
				--
				BEGIN
					SELECT TipoCtto INTO cTipoCtto FROM EQ_TIPOCTTO
				...
				VALUES (
					I.RAMO,I.POLIZA,I.SERIE,I.RECLAM,I.CODBE,I.CODCOB,I.NUMCERT,I.NUMREC,I.FECEMI,I.FECINIVALID,
					I.FECFINVALID,I.FECINIVIG,I.FECFINVIG,I.FECING,I.MIGRA,I.RAZONMIGRA,I.NUMREN,I.TIPOSUSC,
					I.CODRAMOCERT,I.CODCOBERT,I.STSCOBERT,I.BIECOB,I.CLASEBIEN,I.STSBIEN,I.CODMONEDA,I.CODOFIEMI,
					I.CODPROD,I.CODPLAN,I.REVPLAN,I.CODPARENT,I.STSCERT,I.DESCCERT,I.STSPOL,I.TIPOVIG,I.TIPOPDCION,
					I.TIPOFACT,I.INDCOA,I.CODFORMFCION,I.CODOFISUSC,I.CODINTER,I.TIPOMOV,I.PRIMAMONEDA,I.MONTOPRIMA,
					I.SUMAASEGMONEDA,I.SUMAASEG,I.TASACAMBIO,I.PORCCOM,I.MTOCOMISION,I.STSREC,I.STSSIT,I.CLASERIESGO,
					I.CODFORMPAGO,I.INDASEGTITULAR,I.TIPOID,I.NUMID,I.DVID,I.CODCLI,I.CODGRP,I.CODRAMOREA,I.MIGRAREA,
					I.TIPOCOBERT,I.INDACUMULASUMA,I.INDACUMULAPRIMA,I.TIPCON,I.SUMAREA,I.PRIMAREA,I.PORCENTAJE,NSerieREA,
					cTipoCtto,nIdeCttoRea,nOrdenCtto
				);
				COMMIT;
			END;
		END LOOP;
		...
		UPDATE EQ_REASEGURO_REN
		SET MIGRA = 'N',
			RAZONMIGRA = 'IdeCttoRea - nOrdenCtto NULO'
		WHERE MIGRA = 'S'
		AND IdeCttoRea IS NULL;
		COMMIT;
	...
	IS
		...
		FROM (
			SELECT DISTINCT 
				...
				DECODE (PORCENTAJE,100,PORCENTAJE,ROUND (PORCENTAJE,2)) PORCDISTTIPOCTTO,
				CLASERIESGO,CODRAMOREA
			FROM EQ_REASEGURO_REN
			WHERE MIGRA = 'S'
			AND MIGRAREA = 'S'
		...
		SELECT DISTINCT
			...
			NULL PRIMANETACESION,NULL PORCCOM,NULL PORCDEP,NULL INTDEP,NULL DIAPAGO,NULL NROCARTAENTREGA,
			NULL TEXTO,CODINTER,NULL PORIMPMUN,NULL PORAPORSUPER,PM.CONVIERTE_DIAMESANO_FECHA (DIAING,MESING,ANOING) FECINIVIG,
			PM.CONVIERTE_DIAMESANO_FECHA (DIAVEN,MESVEN,ANOVEN) FECFINVIG,NULL INDANULACION,NULL IDECTTOREA,NULL STSNUMCESION,
			NULL FECSTSNUMCESION,NULL NUMOBLIG,NULL NUMACRE,1 NUMGENREA,CODRAMOREA
		...
		SELECT
			...
			NVL((SELECT CodAcepRiesgo FROM EQ_ACEP_RIESGO WHERE codrf = B.Codrf),LPAD (B.Codrf,6,0)) CODACEPRIESGO,
			0 NUMMODDISTREA,B.NUMCES NUMCESION,0 NROGARANTIA,NULL NNUMDIA,NULL FECGARPAGO,
			NULL PORCCANT,NULL FECPAGO,NULL NUMOBLIG,NULL INDPAGO,SERIEREA ANOCESION
		FROM EQ_REASEGURO_REN A,REPF15_REN B
		WHERE A.RAMO = B.RAMO
		AND A.POLIZA = B.POLIZA
		AND A.NUMREC = B.NUMREC
		...
		IS
		SELECT * ROM ACSMIGRE.VIG_DIAS_GAR_PAGO@DESA
		WHERE RAMO_AS = pRAMO
		AND POLIZA_AS = pPOLIZA
		AND NUMGENREA = pNUMGENREA
		AND CODACEPRIESGO = pCODACEPRIESGO
	...
	BEGIN
		...
		LOOP
			...
			IF CUR_VIG_GEN_REA%NOTFOUND THEN
				...
				VALUES (
					I.RAMO_AS,I.POLIZA_AS,I.NUMCERT,I.CODRAMOREA,I.SUMAASEG,I.PRIMAMOV,I.FECINIVIG,
					I.FECFINVIG,I.FECMOV,I.STSGENREA,I.CLASERIESGO,I.ORDEN,I.CODCUMULO,I.FECANUL,
					I.TIPOCODCUMULO,I.NUMREC,I.CODRAMO,I.NUMGENREA,NULL
				);
				COMMIT;
			END IF;
		...
		LOOP
			...
			IF CUR_VIG_DIAS_GAR_PAGO%NOTFOUND THEN
				...
				VALUES (
					...
					I.NNUMDIA,I.FECGARPAGO,I.PORCCANT,I.FECPAGO,I.NUMOBLIG,I.INDPAGO,I.ANOCESIO
				);
				COMMIT;
			END IF;
	...
	IS
		...
		IS
		SELECT DISTINCT
			HJ03.RAMO,HJ03.POLIZA,HJ03.NUMREC,HJ03.CLARCV,HJ03.FECADQ,HJ03.DESTIN,HJ03.ENSAMB,HJ03.CODMOD,
			EQ.NUMCERT NUMCERT,DECODE (HJ03.REMOLQ,1,'NOUSA','SIUSA') AS CODREMOLQUE,HJ03.NUMPLA AS NUMPLACA,
			HJ03.ANOVEH AS ANOVEH,HJ03.TONELA AS CANTTONELADA,HJ03.CANPAS AS NUMPUESTOS,'NOA' AS TIPOVEH,
			HJ03.COLOR AS COLOR,HJ03.SERCAR AS SERIALCARROCERIA,HJ03.SERMOT AS SERIALMOTOR,
			DECODE (HJ03.STSREC,'3','ANU','ACT') AS STSEXP,'968' AS CODINSPEC,'P' AS INDTIPOPLACA,0 AS CANTKMS,'N' AS INDNUEVO,
			NULL AS TIPOCARGA,'S' AS INDREQREINSP,'024' AS CODPLANEXCESO,
			LPAD (TO_CHAR (HJ03.ENSAMB),3,'0')	|| LPAD (TO_CHAR (HJ03.CODMOD),3,'0') ACARGU,TO_NUMBER (EQ.NUMCERT) NUMCERT,EQ.SUMASE
		FROM (
			SELECT
				EQ.RAMO,EQ.POLIZA,EQ.NUMREC,EQ.NUMCERT,EQ.FECING AS FECSTS,
				EQ.CODOFIEMI,HJ.ENSAMB,HJ.CODMOD,MAX (SUMAASEG) SUMASE
		...
		IS
		SELECT DISTINCT
			A.RAMO,A.POLIZA,A.NUMCERT NUMCERT,B.CLARCV,B.DESTIN,B.NUMPLA,B.ANOVEH,B.TONELA,B.CANPAS,B.FECADQ,
			B.COLOR,TRIM (B.SERCAR) SERCAR,B.SERMOT,DECODE (B.REMOLQ,1,'NOUSA','SIUSA') AS CODREMOLQUE,
			'NOA' AS TIPOVEH,'968' AS CODINSPEC,'P' AS INDTIPOPLACA,0 AS CANTKMS,'N' AS INDNUEVO,NULL AS TIPOCARGA,
			'S' AS INDREQREINSP,'024' AS CODPLANEXCESO,TRIM (B.SERMOT) AS SERIALMOTOR,TRIM (B.SERCAR) AS SERIALCARROCERIA,
			B.CANPAS AS NUMPUESTOS,B.TONELA AS CANTTONELADA,B.ANOVEH AS ANOVEH,B.NUMPLA AS NUMPLACA,
			LPAD (TO_CHAR (B.ENSAMB),3,'0')|| LPAD (TO_CHAR (B.CODMOD),3,'0') AS ACARGU,NULL NROCIV,SUMASE
		FROM (
			SELECT
				EQ.RAMO,EQ.POLIZA,EQ.NUMREC,EQ.NUMCERT,EQ.FECING AS FECSTS,
				EQ.CODOFIEMI,CL.ENSAMB,CL.CODMOD,MAX (SUMAASEG) SUMASE
		...
		CLASEVEH 		VARCHAR2 (6);
		CLASEVEHACSEL 	VARCHAR2 (6);
		CODPLANEXCESOA 	VARCHAR2 (3);
		INDFABRIC 		VARCHAR2 (1);
		CODPLANEXCESO 	VARCHAR2 (3);
		VER_NOMBRE 		VARCHAR2 (30);
		CODMODELO 		VARCHAR2 (5);
		CODMARCA 		VARCHAR2 (5);
		CODVERSION 		VARCHAR2 (5);
		VER_TRANSM 		ACSMIGRA.CERT_VEH_MIG.TIPOTRANS@DESA%TYPE;
		NOMBRE 			VARCHAR2 (30);
		EXISTE 			NUMBER;
		ACFUNC 			CTRALTAB.ACFUNC%TYPE;
		VER_MOTOR 		VVALORES.VER_MOTOR%TYPE;
		CODPENAL 		TB_REF_CODPENAL.CODPENAL%TYPE;
		CODVER 			VAPF02.CODVER%TYPE;
		PLANAPOV 		TB_REF_TIPOVEH.PLANAPOV%TYPE;
		NUMPLACA 		ACSMIGRA.CERT_VEH_MIG.NUMPLACA@DESA%TYPE;
		COLOR 			ACSMIGRA.CERT_VEH_MIG.COLOR@DESA%TYPE;
		SERIALCARRO 	ACSMIGRA.CERT_VEH_MIG.SERIALCARROCERIA@DESA%TYPE;
		SERIALMOTOR 	ACSMIGRA.CERT_VEH_MIG.SERIALMOTOR@DESA%TYPE;
		NSUMASE 		NUMBER;
	BEGIN
		...
		LOOP
			BEGIN
				...
				END IF;
				--
				SELECT MAX (CODVER) INTO V_CODVER
				FROM VAPF02
				WHERE PROCES = 'S';
				--
				BEGIN
					SELECT
						TRIM (SX.VER_NOMBRE),TRIM(SUBSTR(LPAD (TRIM (TO_CHAR (SX.VER_CIVI)),8,'0'),4,3)),
				...
				EXCEPTION WHEN OTHERS THEN
					V_VER_TRANSM := 'S';
					V_NOMBRE := PM.ELIMINA_ESPACIOS (
						SUBSTR(TRIM (V_ACFUNC) || 'SIN VERSION',1,30)
					);
				...
				BEGIN
					...
					IF CUR_VIG_CERT_VEH%NOTFOUND THEN
						INSERT INTO ACSMIGRE.VIG_CERT_VEH@DESA (
							RAMO_AS,POLIZA_AS,NUMCERT,CODMODELO,CODMARCA,CODVERSION,DESCMOTOR,CODDESTINADO,CODREMOLQUE,
							NUMPLACA,ANOVEH,CANTTONELADA,NUMPUESTOS,TIPOVEH,FECADQ,COLOR,SERIALCARROCERIA,SERIALMOTOR,
							CLASEVEH,CODPLANAPOV,FECREG,INDFABRIC,INDTIPOPLACA,CANTKMS,TIPOTRANS,INDNUEVO,TIPOCARGA,
							CODPLANEXCESO,INDREQREINSP,DESCTIPOVEH,CODPENAL,TIPODES
						)
						VALUES (
							CO.RAMO,CO.POLIZA,CO.NUMCERT,V_CODMODELO,V_CODMARCA,V_CODVERSION,V_VER_MOTOR,V_CODDESTINADO,
							CO.CODREMOLQUE,V_NUMPLACA,CO.ANOVEH,CO.CANTTONELADA,CO.NUMPUESTOS,V_TIPOVEHACSEL,V_FECADQ,
							V_COLOR,V_SERIALCARRO,V_SERIALMOTOR,V_CLASEVEHACSEL,V_PLANAPOV,NULL,'N',CO.INDTIPOPLACA,
							CO.CANTKMS,V_VER_TRANSM,CO.INDNUEVO,CO.TIPOCARGA,'000',CO.INDREQREINSP,V_NOMBRE,'000',CO.DESTIN
						);
						COMMIT;
					END IF;
		...
		LOOP
			BEGIN
				...
				--
				BEGIN
					SELECT APOV INTO V_PLANAPOV FROM EQ_APOV
				...
				BEGIN
					...
					IF CUR_VIG_CERT_VEH%NOTFOUND THEN
						INSERT INTO ACSMIGRE.VIG_CERT_VEH@DESA (
							RAMO_AS,POLIZA_AS,numcert,codmodelo,codmarca,codversion,descmotor,coddestinado,
							codremolque,numplaca,anoveh,canttonelada,numpuestos,tipoveh,fecadq,color,serialcarroceria,
							serialmotor,claseveh,codplanapov,fecreg,indfabric,indtipoplaca,cantkms,tipotrans,indnuevo,
							tipocarga,codplanexceso,indreqreinsp,desctipoveh,codpenal,tipodes
						)
						VALUES (
							CO.RAMO,CO.POLIZA,CO.numcert,v_codmodelo,v_codmarca,v_codversion,v_ver_motor,v_coddestinado,
							CO.codremolque,v_numplaca,CO.anoveh,CO.canttonelada,CO.numpuestos,v_tipoveh,v_fecadq,v_color,
							v_serialcarro,v_serialmotor,v_claseveh,v_planapov,NULL,'N',CO.indtipoplaca,CO.cantkms,v_ver_transm,
							CO.indnuevo,CO.tipocarga,CO.codplanexceso,CO.indreqreinsp,v_nombre,'000',CO.destin
						);
						COMMIT;
					END IF;
					CLOSE CUR_VIG_CERT_VEH;
	...
	IS
		...
		WITH EQ AS (
			...
			FROM EQ_SINIESTRO_REN WHERE MIGRA = 'S'
		)
		SELECT DISTINCT
			...
			GG.GRALIC AS VGRADOLIC,NVL (TO_NUMBER (SUBSTR(GG.ANOEXP,3,4)),0) AS VANOSEXPER
		...
		SELECT DISTINCT
			...
			'S' AS VINDCONDPRINC,EQ.CODPARENT VCODPARENT,EQ.STSCERT AS VSTSCONDVEH,EQ.FECING AS VFECING,
			0 AS VNUMLIC,GG.GRALIC AS VGRADOLIC,NVL (TO_NUMBER (SUBSTR(GG.ANOEXP,3,4)),0) AS VANOSEXPER
	...
	BEGIN
		BEGIN
			...
			LOOP
				DELETE FROM ACSMIGRE.VIG_COND_VEH@DESA WHERE ROWID = I.ROWID;
				COMMIT;
			END LOOP;
			--
			...
			LOOP
				BEGIN
					...
					IF CUR_VIG_COND_VEH%NOTFOUND THEN
						...
						VALUES (
							MM.RAMO,MM.POLIZA,MM.NUMCERt,MM.CODCLI,MM.VINDCONDPRINC,MM.VCODPARENT,
							MM.VSTSCONDVEH,MM.VFECING,MM.VGRADOLIC,MM.VANOSEXPER,MM.TIPOID,MM.NUMID,MM.DVID
						);
						COMMIT;
					END IF;
			...
			LOOP
				BEGIN
					...
					IF CUR_VIG_COND_VEH%NOTFOUND THEN
						...
						VALUES (
							dd.RAMO,dd.POLIZA,dd.vnumcert,dd.codcli,dd.vindcondprinc,dd.vcodparent,
							dd.vstscondveh,dd.vfecing,dd.vgradolic,dd.vanosexper,dd.tipoid,dd.numid,dd.dvid
						);
						COMMIT;
					END IF;
					CLOSE CUR_VIG_COND_VEH;
	...
	IS
		...
		WITH EQ AS (
			...
			WHERE Migra = 'S'
			AND Ramo NOT IN (32,34,35,36,38,41)
			AND NOT EXISTS (
				SELECT 'X' FROM EQ_POLIZA_NO_MIGRADO
				WHERE MIGRA = 'S'
				AND Ramo = EQ.Ramo
				AND Poliza = EQ.Poliza
				AND CodCob = 6730 AND CodProd = 'AUTI'
			)
	...
	BEGIN
		...
		LOOP
			INSERT INTO ACSMIGRE.VIG_DIREC_COBROPOLIZA@DESA (
				RAMO_AS,POLIZA_AS,Direc,Telef1,Fax,Telef2,Telex,Telef3,CodPais,
				CodEstado,CodCiudad,CodMunicipio,ZonaPostal,ZonaCobro,FecSts
			)
			VALUES (
